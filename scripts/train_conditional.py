...
def main(args):
    ...
    if args.limit_data_size is not None:
        dataset = Subset(dataset, range(args.limit_data_size))
    ...
    gamma_param = torch.tensor(GAMMA_LOOKS, device=model.device).to(weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            speckle = (
                gamma_dist.sample(clean_images.shape)
                .to(weight_dtype)
                .to(clean_images.device)
            )
            speckle_images = clean_images * speckle
            speckle_images = transforms.functional.normalize(
                speckle_images,
                mean=torch.tensor(0.5, device=clean_images.device),
                std=torch.tensor(0.5, device=clean_images.device),
                inplace=True,
            )

            # concatenate in the channel dim
            in_images = torch.cat([noisy_images, speckle_images], dim=1)

            with accelerator.accumulate(model):
                # Predict the noise residual
                model_output = model(in_images, timesteps).sample
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                # First batch to use for visualization
                val_batch = next(iter(val_dataloader))
                val_images = val_batch["input"].to(weight_dtype).to(accelerator.device)

                # Sample speckle noise
                speckle = (
                    gamma_dist.sample(val_images.shape)
                    .to(weight_dtype)
                ...
                if args.logger == "tensorboard":
                    ...
                    tracker.add_images(
                        "val_samples", val_images_processed, epoch
                    )
                    tracker.add_images(
                        "val_samples_speckled", speckle_images_processed, epoch
                    )
                    tracker.add_images(
                        "val_samples_denoised", images_processed.transpose(0, 3, 1, 2), epoch
                    )
                elif args.logger == "wandb":
